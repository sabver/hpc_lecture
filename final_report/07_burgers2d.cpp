...
//结果和Python上面的结果大体一致，但是python版本的变化好像有尖刺，c++的版本更加平缓，原因不明
...
//   printf("convection\n");
  ...
  for(int j=1;j<row-1;j++){
    for(int i=1;i<col-1;i++){
      ...
      u[j][i] = (u_old[j][i] -
                       dt / dx * u_old[j][i] * 
                       (u_old[j][i] - u_old[j][i-1]) - 
                       dt / dy * v_old[j][i] * 
                       (u_old[j][i] - u_old[j-1][i]) + 
                       nu * dt / pow(dx,2) * 
                       (u_old[j][i+1] - 2 * u_old[j][i] + u_old[j][i-1]) + 
                       nu * dt / pow(dy,2) * 
                       (u_old[j+1][i] - 2 * u_old[j][i] + u_old[j-1][i]));
      ...
      v[j][i] = (v_old[j][i] - 
                       dt / dx * u_old[j][i] *
                       (v_old[j][i] - v_old[j][i-1]) -
                       dt / dy * v_old[j][i] * 
                      (v_old[j][i] - v_old[j-1][i]) + 
                       nu * dt / pow(dx,2) * 
                       (v_old[j][i+1] - 2 * v_old[j][i] + v_old[j][i-1]) +
                       nu * dt / pow(dy,2) *
